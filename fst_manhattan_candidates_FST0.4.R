...
# Generate a Manhattan plot of Weir & Cockerham FST values with NC_ chromosome labels
...
fst$CHROM <- factor(fst$CHROM, levels = chrom_order)
cumsum_sizes <- cumsum(c(0, chrom_sizes[-length(chrom_sizes)]))
names(cumsum_sizes) <- chrom_order
fst[, BPcum := POS + cumsum_sizes[as.character(CHROM)]]

axisdf <- fst[, .(center = mean(BPcum)), by = CHROM]

# ---- Step 4: Select candidate SNPs ----
# Use a fixed threshold for highly differentiated SNPs
fst_threshold <- 0.4
candidates <- fst[WEIR_AND_COCKERHAM_FST >= fst_threshold]

# Save candidate SNPs (CHROM + POS) for annotation
fwrite(candidates[, .(CHROM, POS)], "candidate_SNPs_FST0.4.csv")
...
manhattan_plot <- ggplot(fst, aes(x = BPcum, y = WEIR_AND_COCKERHAM_FST)) +
  ...
  scale_color_manual(values = rep(c("grey30","grey70"), length.out = length(chrom_order))) +
  theme_bw() +
  theme(
  ...
  ggtitle("FST Manhattan Plot (FST >= 0.4 Highlighted)")

ggsave("FST_Manhattan_candidates_FST0.4.png", plot = manhattan_plot, width = 12, height = 6)
